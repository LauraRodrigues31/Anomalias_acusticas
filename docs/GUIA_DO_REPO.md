...
10. **Qual a latência do sistema?** Inerente: 64 ms de janela (passo de 32 ms) e ≥ 160 ms do voto 6 de 8. No ESP32 (medição real de 21/09/2026, silêncio com alarmes ocasionais): `t_total` média 1452 µs por janela (não inclui os 64 ms da janela; ≈ 4,5% do passo de 32 ms), `t_feat` média 1379 µs, `t_decision` p50 ≈ 161,6 ms, sem overruns/drops. As do PC (`t_feat` ≈ 17 µs de mediana) não valem para o ESP32.
